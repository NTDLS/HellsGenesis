...
namespace HG.Types
{
    internal class HgNormalizedAngle
    {
        ...
        public static HgPoint<double> ToXY(HgNormalizedAngle angle)
        {
            return new HgPoint<double>(angle.X, angle.Y);
        }

        public static HgPoint<double> DegreesToXY(double degrees)
        {
            double radians = DegreesToRadians(degrees) - RadianOffset;
            return new HgPoint<double>(Math.Cos(radians), Math.Sin(radians));
        }

        public static HgPoint<double> RadiansToXY(double radians)
        {
        ...
        {
            return (Math.Round(((HgNormalizedAngle)o).X, 4) == this.X && Math.Round(((HgNormalizedAngle)o).Y, 4) == this.Y);
        }
