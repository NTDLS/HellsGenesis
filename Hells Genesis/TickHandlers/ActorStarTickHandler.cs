...
namespace HG.TickHandlers
{
    internal class ActorStarTickHandler : IVectoredTickManager
    {
        ...
        public ActorStarTickHandler(EngineCore core, EngineActorController manager)
        {
            _core = core;
            _controller = manager;
        }
        ...
        public void ExecuteWorldClockTick(HgPoint displacementVector)
        {
            if (displacementVector.X != 0 || displacementVector.Y != 0)
            {
                ...
                if (_controller.VisibleOfType<ActorStar>().Count < _core.Settings.DeltaFrameTargetStarCount) //Never wan't more than n stars.
                {
