...
namespace NebulaSiege.Managers
{
    ...
    internal class EngineAssetManager
    {
        ...
        /// <returns></returns>
        public NsAudioClip GetAudio(string assetRelativePath, float initialVolumne, bool loopForever = false)
        {
            string assetAbsolutePath = Path.Combine(assetRawPath, assetRelativePath).Trim().Replace("\\", "/");
            ...
            lock (_collection)
            {
                if (_collection.TryGetValue(key, out var value))
                {
                    return value as NsAudioClip;
                }

                using (var stream = new FileStream(assetAbsolutePath, FileMode.Open, FileAccess.Read))
                {
                    var result = new NsAudioClip(stream, initialVolumne, loopForever);
