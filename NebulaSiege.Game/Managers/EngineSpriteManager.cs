...
using NebulaSiege.Game.Sprites.Player;
...
namespace NebulaSiege.Game.Managers
{
    ...
    internal class EngineSpriteManager
    {
        ...
        private NsPoint _radarScale;
        private NsPoint _radarOffset;
        ...
        public List<SpriteBase> Intersections(SpriteBase with)
        {
            lock (Collection)
            {
                ...
                foreach (var obj in Collection.Where(o => o.Visable == true))
                {
                    if (obj != with)
                    {
                        if (obj.Intersects(with.Location, new NsPoint(with.Size.Width, with.Size.Height)))
                        {
                            objs.Add(obj);
                        }
                    }
        ...
        public void RenderPostScaling(SharpDX.Direct2D1.RenderTarget renderTarget)
        {
            ...
            if (RenderRadar)
            {
                ...
                if (_radarScale == null)
                {
                    ...
                    _radarScale = new NsPoint(radarBgImage.Size.Width / radarVisionWidth, radarBgImage.Size.Height / radarVisionHeight);
                    _radarOffset = new NsPoint(radarBgImage.Size.Width / 2.0, radarBgImage.Size.Height / 2.0); //Best guess until player is visible.
                }
